...
SegmentData.f <- function(sightings, effort, trunc.dist=3000, truncate_sdata=T, along.track.dist.m=c(2000,5000,10000), 
...
{
  # 
  # Wrapper around N.Kelly code to segment data for dsm
  # D.Peel
  ...
  for (idist in along.track.dist.m)
  {
    ...
    segment.summaries.all[[iname]]<- transect.segmentation.f( effort.dataset = effort.2rows, transect.sub.name = transect.sub.name , lon.col = lon.col, lat.col = lat.col, 
    ...
    unique.segment.no[[iname]]<- unlist(lapply( sightings$sighting.no.unique, segment.no.per.sighting.f, segment.summary = segment.summaries.all[[iname]], 
    ...
    # SUMMARISE SEGMENTS -----------------------------------------------------------  
    summarise.sightings.at[[iname]]<- summarise.sightings.at.segment.f( segment.data = segment.summaries.all[[iname]], sighting.summary.data = sightings, 
                                                                        sighting.summary.data.column = datacolm, trunc.dist = trunc.dist, size.name = size.name) 
    
    summaries[[iname]] <- cbind( segment.summaries.all[[iname]], summarise.sightings.at[[iname]]) 
    
    # Hack to rename a couple of columns so it matches what DSM expects - DP
    colnames(summaries[[iname]])[match("unique.segment.no",colnames(summaries[[iname]]))] <- "Sample.Label"
    colnames(summaries[[iname]])[match("segment.length.m",colnames(summaries[[iname]]))] <- "Effort"
    
    if (!is.null(xyprojection))
    {
      tem<- fn.projection(summaries[[iname]]$lon.mid, summaries[[iname]]$lat.mid,ToPrj = xyprojection) 
      summaries[[iname]]$x <- tem[[1]]
      summaries[[iname]]$y <- tem[[2]]
    }
    else
    {
      summaries[[iname]]$x <- summaries[[iname]]$lon.mid
      summaries[[iname]]$y <- summaries[[iname]]$lat.mid
    }
    
    # Link seg data to sightdata to make obs data
    id <- match(sightings[ , datacolm], summaries[[iname]]$Sample.Label)
    
    
    obsdata[[iname]] <- data.frame(object=sightings$sighting.no.unique, Sample.Label=sightings[ , datacolm], 
                                   size=sightings$Count, distance=sightings$distance, Effort=summaries[[iname]]$Effort[id], Area=sightings$Area, Migration=sightings$Migration, stringsAsFactors = F)
    
  }
  
  ans <- list(data=list(sightings=sightings, effort= effort.2rows), segdata=summaries, obsdata=obsdata)
